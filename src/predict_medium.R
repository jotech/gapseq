...
spec <- matrix(c(
  ...
  'manual.conc', 'c', 2, "character", "(optional, character) Specify fixed maxFlux
...
if ( is.null(opt$manual.conc) ) { opt$manual.conc = NULL }
if ( is.null(opt$output.file) ) { opt$output.file = NULL }

#' Example for manual.conc: "cpd00007:18.5;cpd00011:0" for a medium with allowed
#' oxygen (cpd00007) uptake (18.5) but without CO2 (cpd00011)
predict_medium <- function(mod, pathway.pred, manual.conc = NULL) {
  ...
  if(!is.null(manual.conc)) {
    manual.conc <- unlist(str_split(manual.conc, ";"))
    manual.conc <- str_split(manual.conc, ":")
    for(i in 1:length(manual.conc)) {
      if(length(manual.conc[[i]]) != 2) {
        stop("something is wrong with the 'manual.conc' string. Please follow this pattern:
             \t \"[Compound 1]:[Compound 1 maxFlux];[Compound 2]:[Compound 2 maxFlux]\"
             \t e.g. \"cpd00007:18.5;cpd00011:0\"")
      }
      
      if(manual.conc[[i]][1] %in% medium_rules$cpd.id) {
        medium_rules[cpd.id == manual.conc[[i]][1], maxFlux := as.numeric(manual.conc[[i]][2])]
      } else {
        dttmp <- data.table(cpd.id = manual.conc[[i]][1],
                            maxFlux = as.numeric(manual.conc[[i]][2]),
...
manual.conc      <- opt$manual.conc
output.file      <- opt$output.file

dt_medium <- predict_medium(mod, pathway.pred, manual.conc)

if(is.null(output.file))
  output.file <- gsub("\\.RDS$|-draft\\.RDS$|\\.xml$|-draft\\.xml$",
                      "-medium.csv", opt$model)

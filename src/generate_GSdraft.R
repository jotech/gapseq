...
spec <- matrix(c(
  ...
  'biomass', 'b', 2, "character", "Biomass reaction for network model. Options: Gram \"pos\" OR \"neg\" OR \"archaea\" OR \"auto\"? Default: \"auto\". See Details for biomass reaction prediction.",
...
if ( !is.null(opt$help) | is.null(opt$blast.res) | (is.null(opt$biomass) & is.null(opt$genome.seq)) | (!is.null(opt$biomass) && opt$biomass=="auto" && is.null(opt$genome.seq))) {
  ...
  cat("\n")
  cat("Details:\n")
  cat("\"biomass\" (-b): Please note that if set to \"auto\", the external programms barrnap, blastn, and bedtools are required as well as argument \"-c\" for genome sequence. The Biomass is predicted based on the 16S rRNA gene sequence using a classifier that was trained on 16S rRNA genes from organisms with known Gram-staining phenotype.",
      "In case no 16S rRNA gene was found, a k-mer based classifier is used instead.\n")
  q(status=1)
}

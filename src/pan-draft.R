...
spec <- matrix(c(
  ...
  'min.rxn.freq.in.mods', 't', 2, "numeric", "minimum percentage of models that must have a specific reaction for that to be included in the pan-Draft. Default: 0.08",
  ...
  'only.binary.rxn.table','b', 2, "logical", "perform only models comparison to get a binary table summarizing rxn presence/absence in the models. Default: FALSE",
...
if ( is.null(opt$min.rxn.freq.in.mods) ) { opt$min.rxn.freq.in.mods = 0.08 }
...
if ( !only.binary.rxn.table ){
  ...
  if ( !quick ){
    ...
    # The Heaps law model is fitted to the number of new gene clusters observed when genomes are ordered in a random way. 
    # The model has two parameters, an intercept and a decay parameter called alpha. If alpha>1.0 the pan-genome is closed, if alpha<1.0 it is open.
    suppressMessages(rar.tbl <- rarefaction(rxn2mod_rarefaction_df, n.perm = n.perm)) # lib::micropan --- "New names" message is generated here
    data <- rar.tbl %>% 
      pivot_longer(cols = -Genome, names_to = "Permutation", values_to = "Clusters") %>% 
      filter(Genome > 0) 
    # Heaps' law
    initial_k_estimate <- 1. # randomly set initial estimates
    initial_gamma_estimate <- 1.
    heaps_model <- function(n, k, gamma) {k * n^gamma}
    fit <- nls(Clusters ~ heaps_model(Genome, k, gamma), 
                data = data,
                start = list(k = initial_k_estimate, gamma = initial_gamma_estimate))
    k_estimate <- coef(fit)["k"] # parameter estimates
    gamma_estimate <- coef(fit)["gamma"]

    # save statictics
    stat <- c(dim(rxn2mod_dt)[2]-1, dim(rxn2mod_dt)[1], strict_core_rxn_num, core_rxn_num, shell_rxn_num, cloud_rxn_num, h.est[1], h.est[2], k_estimate, gamma_estimate)
    stat_dt <- data.table(t(stat))
    colnames(stat_dt) <- c("nGenome", "total_RXN", "strict_core", "core", "shell", "cloud", "Power_law_k_estimate", "Power_law_alpha_estimate", "Heaps_law_k_estimate", "Heaps_law_gamma_estimate")
    fwrite(stat_dt, file = file.path(output.dir,"pan-reactome_stat.tsv"), sep = "\t")
    
    vars_to_remove <- c("rxn2mod_rarefaction_df", "h.est", "data", "fit", "strict_core_rxn_df", "core_rxn_df", "shell_rxn_df")
    rm(list = vars_to_remove) # Remove some variables
  } else {
  ...
  rxn_PresAbs_dt <- copy(rxn2mod_dt)
  th_list <- seq(0., 1., by = 0.01) 
  for (th in th_list){
    cat(paste("\nth: ", th))
    col_n <- paste0("pan.mod_", th)
    # th_rxn_df <- subset(rxn2mod_dt, rowSums(rxn2mod_dt[, ..mod_ids]) >= round(num.mod*th)) # Reactions present/absence in pan-Draft based on frequency th
    # rxn_PresAbs_dt[, (col_n) := as.integer(rxn %in% th_rxn_df$rxn)] # add column on dataset 
    # row_sub <- rxn_PresAbs_dt[,  ..col_n]==1   
    # subSet_rxn_dt <- rxn_PresAbs_dt[row_sub[, 1], .SD, .SDcols = "rxn"] # select id of present reactions
    # pan.mod <- build_panDraft(subSet_rxn_dt, info_all_rxns_mods, first_mod_desc) # build pan-Draft
    # saveRDS(pan.mod, file.path(output.dir,paste0(th,"_panModel-draft.RDS")))
    ...
    # alternative: "median(weight)" do not consider missing RXN and compute only the once present
    #              "custom_median(weight, num.pan, num.mod)" consider missing RXN and compute the median based on the total number input models
    weights_dt[, weight.pan := custom_quartile_weight(weight, num.pan, num.mod, th), by = .(seed)]
    weights_dt[, num.pan := NULL] # drop the colum num.pan
    weights_dt <- weights_dt[order(seed, weight)] # alternative: "abs(weight - weigth.pan)" 
    weights_dt <- weights_dt[!duplicated(seed)] # keep the entry with the highest score.
    weights_dt[,c("weight", "weight.pan")] <- weights_dt[,c("weight.pan", "weight")] # Swap the values of "weight" and "weight.pan" columns
    colnames(weights_dt)[colnames(weights_dt)=="weight.pan"] <- "weight.old" # Rename column
    setkeyv(weights_dt, c("model_id", "seed")) # set data.table keys
    saveRDS(weights_dt, file.path(output.dir,paste0(th, "_panModel-rxnWeigths.RDS")))

    ### Update rxnXGenes table  
    xgenes_dt <- rbindlist(xgenes_list, idcol = "model_id")
    setkeyv(xgenes_dt, c("model_id", "seed"))
    xgenes_dt <- xgenes_dt[weights_dt] # extract only genes corresponding to the ref reactions
    saveRDS(xgenes_dt, file.path(output.dir,paste0(th, "_panModel-rxnXgenes.RDS")))

    ### Save the list of detected Pathways
    pathways_dt <- do.call(rbind, pathways_list)
    pathways_dt <- pathways_dt[, .(Prediction = sum(Prediction)), by=c("ID", "Name")]
    pathways_dt$Prediction <- pathways_dt$Prediction > num.mod*th
    fwrite(pathways_dt, file = file.path(output.dir,paste0(th,"_panModel-tmp-Pathways.tbl")), sep = "\t", quote = FALSE)
  } 
}
...
spec <- matrix(c(
  ...
  'min.rxn.freq.in.mods', 't', 2, "numeric", "minimum reaction frequency (mrf) to include the reactions in the pan-Draft. Default: 0.06",
  ...
  'sbml.no.output', 's', 2, "logical", "Do not save model as sbml file. Default: Save as SBML"
...
if ( !only.binary.rxn.tbl ){
  weights_list <- lapply(model_list, FUN = function(mod) { return(mod@metadata$rxnWeights)})
  xgenes_list <- lapply(model_list, FUN = function(mod) { return(mod@metadata$rxnXgenes)})
...
if ( !only.binary.rxn.tbl ){
  if(length(model_list)==length(weights_list) & length(model_list) == length(xgenes_list) & length(model_list) == length(pathways_list)){
    ...
    stop("Check you input, the number of loaded files per data type is different.")
  }
  ...
  vars_to_remove <- c("xgenes_list", "weights_list", "pathways_dt")
...
if ( !only.binary.rxn.tbl ){
  ...
  pan.mod@metadata <- list(rxnWeights = weights_dt,
                           rxnXgenes = xgenes_dt)
  ...
  if(!opt$sbml.no.output){
    ...
    write_gapseq_sbml(pan.mod, file.path(output.dir,"panModel-draft"))
  }
}
